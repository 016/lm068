/**
 * Backend Admin Main JavaScript v12 - 支持多列tooltip配置数组和多选框自动处理
 * 基于 main_11.js 优化，tooltipConfig支持多个列的数组配置
 *
 * 主要更新：
 * - tooltipConfig现在支持数组格式，可配置多个列的tooltip
 * - applyTooltipConfig()方法支持遍历多个tooltip配置
 * - 向下兼容单个配置对象的格式
 * - multiSelectColumns配置数组支持，自动处理多个多选框列的筛选
 *
 * tooltip配置格式：
