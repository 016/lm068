/**
 * 合集编辑页面 JavaScript - 第12版
 * 基于 collection_edit_6.js 重构，支持 collection/create 统一处理GET和POST请求
 */
...
class CollectionEditManager {
    ...
    initializePageMultiSelects() {
        ...
        // 将选中的ID转换为对象数组，因为JS需要完整的对象
        const selectedContentObjects = this.contentList.filter(video => 
            this.selectedContentIds.includes(video.id) || this.selectedContentIds.includes(video.id.toString())
        );
        ...
        const videosInstance = this.formUtils.initializeMultiSelect('videos', {
            ...
            selected: selectedContentObjects,
            allowClear: true
        });
