...
class HasMany extends Relation
{
    public function get()
    {
        ...
        $sql = "SELECT * FROM {$table} WHERE {$this->foreignKey} = :value";
        $results = $db->fetchAll($sql, ['value' => $localValue]);
    ...
    public function eagerLoad(array $models): void
    {
        ...
        foreach ($results as $row) {
            ...
            if (!isset($relatedMap[$foreignValue])) {
                $relatedMap[$foreignValue] = [];
            }
            $relatedMap[$foreignValue][] = $model;
        }
        ...
        foreach ($models as $model) {
            ...
            $model->$relationName = $relatedMap[$localValue] ?? [];
        }
    }
