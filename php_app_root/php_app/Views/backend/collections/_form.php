    ...
    <div class="form-body">
        ...
        <form id="collectionEditForm" action="<?= $formAction ?>" method="POST">
            ...
            <div class="form-section">
                ...
                <div class="row">
                    <?php if (!$collection->isNew): ?>
                        <?= FormFieldBuilder::for($collection, 'id')
                            ->label('合集ID')
                            ->disabled()
                    ...
                    <?= FormFieldBuilder::for($collection, 'preview')
                        ->type('preview')
                        ->label('合集预览')
                        ->helpText('实时预览合集显示效果')
                        ->render() ?>
                ...
                <div class="row">
                    <?= FormFieldBuilder::for($collection, 'name_cn')->label('中文标题')->render() ?>
                    <?= FormFieldBuilder::for($collection, 'name_en')->label('英文标题')->render() ?>
                </div>

                <div class="row">
                    <?= FormFieldBuilder::for($collection, 'color_class')
                        ->type('select')
                        ->label('合集颜色')
                        ->options([
                    ...
                    <?= FormFieldBuilder::for($collection, 'icon_class')
                        ->label('图标样式')
                        ->placeholder('请输入 Bootstrap 图标类名，如 bi-collection')
                        ->render() ?>
                </div>

                <div class="row">
                    <?= FormFieldBuilder::for($collection, 'content_ids')
                        ->type('multi-select')
                        ->label('关联视频')
            ...
            <div class="form-section">
                ...
                <div class="row">
                    <?= FormFieldBuilder::for($collection, 'short_desc_cn')->label('中文简介')->render() ?>
                    <?= FormFieldBuilder::for($collection, 'short_desc_en')->label('英文简介')->render() ?>
                </div>

                <?= FormFieldBuilder::for($collection, 'desc_cn')
                    ->type('textarea')
                    ->label('中文描述')
                    ->placeholder('请输入合集的详细中文描述...')
                    ->rows(3)
                    ->cssClass('')
                    ->render() ?>

                <?= FormFieldBuilder::for($collection, 'desc_en')
                    ->type('textarea')
                    ->label('英文描述')
                    ->placeholder('Please enter the detailed English description of the collection...')
                    ->rows(3)
                    ->cssClass('')
                    ->render() ?>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="row">
                    <?= FormFieldBuilder::for($collection, 'status_id')
                        ->type('switch')
                        ->label('显示状态')
                        ->value(CollectionStatus::ENABLED->value)
                        ->render() ?>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="row">
                    <?= FormFieldBuilder::for($collection, 'created_at')->label('创建时间')->disabled()->render() ?>
                    <?= FormFieldBuilder::for($collection, 'updated_at')->label('最后更新时间')->disabled()->render() ?>
                </div>
            </div>
            <?php endif; ?>
