/**
 * Tag List Page Enhanced JavaScript v9
 * 标签列表页面增强版JavaScript - 实现整个表头可点击排序功能
 * 基于 tag_list_8.js 更新，新增：
 * - 整个table-cell都可点击触发排序
 * - 循环排序逻辑：asc -> desc -> asc
 * - 改进的排序状态管理
 */
...
function initTagListPage() {
    ...
    setupRefreshButton();
    setupItemsPerPage();
    setupEnhancedSorting(); // 使用增强的排序功能
    setupSelectAll();
    setupBulkActions();
...
function setupEnhancedSorting() {
    ...
    sortableHeaders.forEach(header => {
        ...
        header.addEventListener('click', function(e) {
            ...
            // 获取当前列的排序状态
            const nextDirection = getNextSortDirection(columnId);
            
            console.log(`点击排序列: ${columnId}, 下一个方向: ${nextDirection}`);
            
            // 执行排序
            performSort(columnId, nextDirection);
        });
        
        // 添加悬停效果样式
        header.style.cursor = 'pointer';
        header.title = `点击排序 ${getColumnDisplayName(columnId)}`;
    });
...
/**
 * 获取下一个排序方向
 * 循环逻辑：无排序 -> asc -> desc -> asc -> desc...
 */
function getNextSortDirection(columnId) {
 ...
 * 执行排序
 */
function performSort(columnId, direction) {
    const TableOps = window.AdminCommon.TableOperations;
    
    console.log(`执行排序: ${columnId} ${direction}`);
    
    // 更新排序状态
    currentSort = { field: columnId, direction };
    
    // 对过滤后的数据进行排序
    filteredData = TableOps.sortData(filteredData, columnId, direction, tableColumns);
    ...
    // 更新排序图标状态
    updateSortIcons(columnId, direction);
}
