...
class MultiSelectDropdown {
    ...
    // Helper method to compare IDs regardless of type (string vs number)
    idsEqual(id1, id2) {
        return String(id1) === String(id2);
    }

    // Helper method to find item by ID regardless of type
    findItemById(array, id) {
        return array.find(item => this.idsEqual(item.id, id));
    }

    // Helper method to check if ID exists in array regardless of type
    hasItemWithId(array, id) {
        return array.some(item => this.idsEqual(item.id, id));
    }
    ...
    renderItems() {
        return this.filteredData.map(item => {
            const isSelected = this.hasItemWithId(this.selectedItems, item.id);
            return `
    ...
    bindEvents() {
        ...
        this.elements.list.addEventListener('click', (e) => {
            ...
            if (item) {
                ...
                const isCurrentlySelected = this.hasItemWithId(this.selectedItems, id);
                
                // 如果点击的是checkbox，阻止默认行为让我们来处理
                if (checkbox) {
                    e.preventDefault(); // 防止checkbox默认行为导致页面跳动
                }
    ...
    addItem(id) {
        const dataItem = this.findItemById(this.options.data, id);
        if (dataItem && !this.hasItemWithId(this.selectedItems, id)) {
    ...
    removeItem(id) {
        this.selectedItems = this.selectedItems.filter(item => !this.idsEqual(item.id, id));
        this.updateDisplay();
        this.updateListItem(id, false);
        
        // 触发变更事件
        const removedItem = this.findItemById(this.options.data, id);
        this.container.dispatchEvent(new CustomEvent('multiselect:change', { 
            detail: { 
                action: 'remove',
                item: removedItem,
    ...
    updateListItem(id, selected) {
        ...
        if (item) {
            ...
            // 使用requestAnimationFrame确保DOM更新同步，防止状态不一致
            requestAnimationFrame(() => {
    ...
    updateAllListItems() {
        ...
        requestAnimationFrame(() => {
            items.forEach(item => {
                const id = item.dataset.id;
                const selected = this.hasItemWithId(this.selectedItems, id);
                const checkbox = item.querySelector('.multi-select-checkbox');
                checkbox.checked = selected;
                item.classList.toggle('selected', selected);
            });
    ...
    setValue(value) {
        if (typeof value === 'string') {
            ...
            const items = this.options.data.filter(item => 
                ids.some(id => this.idsEqual(item.id, id))
            );
