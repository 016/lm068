/**
 * Backend Admin Main JavaScript v11 - 支持TableManager tooltip配置
 * 基于 main_10.js 优化，为TableManager添加tooltip配置支持
 * 
 * 主要更新：
 * - TableManager构造函数支持tooltipConfig参数
 * - 在updateDisplay()方法中自动应用tooltip配置
 * - 确保tooltip在排序、分页后持续生效
 * 
 * 其他功能保持与 main_10.js 相同：
 * - 完整的 switch 组件交互功能
 * - 优化的表格管理和排序功能
 * - 统一的事件管理和下拉菜单逻辑
 */
...
class TableManager {
    ...
    updateDisplay() {
        ...
        // 3. v11 新增：自动应用tooltip配置
        this.applyTooltipConfig();
    ...
    applyTooltipConfig() {
        if (this.config.tooltipConfig) {
            console.log('应用TableManager tooltip配置:', this.config.tooltipConfig);
            ...
            console.log('TableManager tooltip配置应用完成');
        }
    }
