/**
 * 表单工具类 - 通用表单功能集合 - 第3版
 * 基于 form_utils_2.js，增强隐藏输入变更检测功能
 * 适用于管理后台的所有表单页面
 * 
 * 功能模块：
 * - 表单验证
...
class FormUtils {
    constructor(formSelector, options = {}) {
        ...
        this.options = {
            ...
            enableHiddenInputTracking: options.enableHiddenInputTracking !== false, // 新增：隐藏输入跟踪
            hiddenInputPollingInterval: options.hiddenInputPollingInterval || 1000, // 新增：轮询间隔
            ...options
        };
        ...
        // 新增：隐藏输入跟踪相关属性
        this.hiddenInputValues = new Map(); // 存储隐藏输入的初始值和当前值
        this.hiddenInputPollingTimer = null; // 轮询定时器
        this.hiddenInputObserver = null; // MutationObserver实例
     ...
     *     enableHiddenInputTracking: true
     * });
     */
    init() {
        this.bindFormEvents();
        ...
        // 新增：初始化隐藏输入跟踪
        if (this.options.enableHiddenInputTracking) {
            this.initializeHiddenInputTracking();
        }
     ...
     * 初始化隐藏输入跟踪系统
     * 使用多种方法来检测隐藏输入的变更：
     * 1. MutationObserver 监听属性变化
     * 2. 定时轮询检查值变化
     * 3. 重写 value 属性的 setter
     */
    initializeHiddenInputTracking() {
        if (!this.form) return;
        ...
        hiddenInputs.forEach(input => {
            this.hiddenInputValues.set(input, {
                initial: input.value,
                current: input.value,
                name: input.name || input.id || 'unnamed'
            });

            // 2. 重写value属性的setter来捕获程序化的值变更
            this.interceptValueSetter(input);
        });

        // 3. 设置MutationObserver监听属性变化
        this.setupHiddenInputMutationObserver(hiddenInputs);

        // 4. 设置定时轮询作为后备方案
        this.setupHiddenInputPolling(hiddenInputs);

        console.log(`初始化隐藏输入跟踪，监控 ${hiddenInputs.length} 个隐藏输入框`);
    }

    /**
     * 拦截input元素的value属性setter
     * 当JavaScript代码修改input.value时，能够捕获到变更
     */
    interceptValueSetter(input) {
        const originalDescriptor = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value');
        const self = this;
        
        // 为每个input元素创建独立的属性描述符
        Object.defineProperty(input, 'value', {
            ...
            set: function(newValue) {
                ...
                // 检查是否为我们监控的隐藏输入
                if (this.type === 'hidden' && self.hiddenInputValues.has(this)) {
                    self.onHiddenInputValueChange(this, oldValue, newValue);
                }
            },
            configurable: true,
            enumerable: true
    ...
    /**
     * 设置MutationObserver监听隐藏输入的属性变化
     */
    setupHiddenInputMutationObserver(hiddenInputs) {
        if (!window.MutationObserver) {
            console.warn('MutationObserver 不支持，将仅使用轮询方式监听隐藏输入变更');
            return;
        }

        this.hiddenInputObserver = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                if (mutation.type === 'attributes' && mutation.attributeName === 'value') {
                    const input = mutation.target;
                    if (input.type === 'hidden' && this.hiddenInputValues.has(input)) {
                        const storedData = this.hiddenInputValues.get(input);
                        const newValue = input.value;
                        const oldValue = storedData.current;
                        
                        if (oldValue !== newValue) {
                            this.onHiddenInputValueChange(input, oldValue, newValue);
                        }
                    }
                }
            });
    ...
    /**
     * 设置定时轮询检查隐藏输入值变化
     * 作为MutationObserver的补充，确保能捕获所有变更
     */
    setupHiddenInputPolling(hiddenInputs) {
        if (this.hiddenInputPollingTimer) {
            clearInterval(this.hiddenInputPollingTimer);
        }

        this.hiddenInputPollingTimer = setInterval(() => {
            hiddenInputs.forEach(input => {
                if (this.hiddenInputValues.has(input)) {
                    const storedData = this.hiddenInputValues.get(input);
                    const currentValue = input.value;
                    
                    if (storedData.current !== currentValue) {
                        this.onHiddenInputValueChange(input, storedData.current, currentValue);
                    }
                }
            });
        }, this.options.hiddenInputPollingInterval);
    }
     ...
     * 当检测到隐藏输入值变更时调用
     */
    onHiddenInputValueChange(input, oldValue, newValue) {
        // 更新存储的当前值
        const storedData = this.hiddenInputValues.get(input);
        if (storedData) {
            storedData.current = newValue;
        }
        ...
        // 触发自定义事件，允许外部监听
        const changeEvent = new CustomEvent('formutils:hiddeninput:change', {
            detail: {
                input: input,
                name: input.name || input.id,
                oldValue: oldValue,
                newValue: newValue,
                isInitialValue: newValue === storedData?.initial
            }
        });
        
        this.form.dispatchEvent(changeEvent);

        console.log(`隐藏输入变更检测: ${input.name || input.id} 从 "${oldValue}" 变更为 "${newValue}"`);
    }
     ...
     * 返回变更情况的详细信息
     */
    getHiddenInputChanges() {
        const changes = [];
        
        this.hiddenInputValues.forEach((data, input) => {
    ...
    /**
     * 检查表单是否有任何修改（包括隐藏输入）
     * 重写原有的 isModified 检查逻辑
     */
    hasFormModifications() {
        // 检查隐藏输入是否有变更
        const hiddenChanges = this.getHiddenInputChanges();
    ...
    populateForm(data) {
        Object.entries(data).forEach(([key, value]) => {
            ...
            if (field) {
                ...
                // 如果是隐藏输入，也需要更新我们的跟踪数据
                if (field.type === 'hidden' && this.hiddenInputValues.has(field)) {
                    const storedData = this.hiddenInputValues.get(field);
    ...
    destroy() {
        ...
        // 清理MutationObserver
        if (this.hiddenInputObserver) {
            this.hiddenInputObserver.disconnect();
        }
