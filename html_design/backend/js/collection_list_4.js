/**
 * Collection List Page JavaScript v4 - 优化tooltip重新初始化
 * 基于 collection_list_3.js 优化，解决分页和排序后tooltip失效问题
 * 
 * 主要修改：
 * - 移除手动tooltip重新初始化逻辑，TableManager现在会自动处理
 * - 优化initCollectionSpecificFeatures函数的tooltip配置
 * - 添加tooltip重新初始化的触发机制
 * - 确保在任何内容更新后tooltip都能正常工作
 */
...
function initCollectionListPage() {
    console.log('=== 初始化合集列表页面（tooltip优化版）===');
    ...
    console.log('=== 合集列表页面初始化完成（tooltip优化版）===');
}
...
function initCollectionSpecificFeatures() {
    ...
    // 合集描述tooltip功能 - 使用公共方法
    // 注意：这里不需要设置reinitialize为true，因为TableManager会自动处理
